"""Training a CNN on MNIST in with DP-SGD optimizer."""
...
flags.DEFINE_integer('microbatches', 8, 'Number of microbatches (must evenly divide batch_size)')
...
def main(_):
  ...
  if FLAGS.dpsgd:
    ...
    opt = tf.optimizers.SGD(learning_rate=FLAGS.learning_rate)
  ...
  for epoch in range(FLAGS.epochs):
    ...
    for (_, (images, labels)) in enumerate(dataset.take(-1)):
      with tf.GradientTape(persistent=True) as gradient_tape:
        ...
        if FLAGS.dpsgd:
          ##### minimize
          grads_and_vars = opt.compute_gradients(loss_fn, var_list, gradient_tape=gradient_tape)
          opt.apply_gradients(grads_and_vars)
          #####
        else:
          grads = gradient_tape.gradient(loss_fn(), var_list)
    ...
    print('Test accuracy after epoch {:d} is: {:.3f}'.format(epoch, test_accuracy))

    # Compute the privacy budget expended so far.
    if FLAGS.dpsgd:
      eps = compute_epsilon((epoch + 1) * steps_per_epoch)
      print('For delta = {}, the current epsilon is: {:.2f}'.format(delta, eps))
    else:
